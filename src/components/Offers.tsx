...
dispatch(fetchdata());
    ...
    return(
        <div className="Main  gap-2  mt-32">
            <div className="container mx-auto border border-gray-400 mt-10 ">
                <div className="cards m-4 grid grid-cols-3  max-[550px]:grid-cols-2  max-[320px]:grid-cols-1 gap-2" >

                {
                meals.length !==0 ?<>{
                meals.filter((meal:Meals)=> meal.isOffer ==="YES")
                    .map((meal:Meals)=>{
                        return(
                        <div key={meal.id} className={`group card relative overflow-hidden   bg-gray-100 duration-700 ease-in-out w-full border`}>
                            
                            <div className="title group-hover:left-0 z-10 ">
                                <h2>{meal.title}</h2>
                            </div>
                            <div className="img overflow-hidden hover:brightness-75 ">
